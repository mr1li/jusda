...
def predict(data,model,mode,con_len,pre_len,save_model_path,sku,yanzheng,end):
    if yanzheng==True:
        ...
        name2=save_model_path + model + '_' + mode + '_' + str(con_len) + '_' + str(pre_len) + '画图数据'+'.csv'
        ...
        def pinjie(group,pre,end,smape):
            ...
            group = group.loc[group.index.repeat(len(pre1[0]))].reset_index(drop=True)
            group['pre']=pre1[t]
            group['real'] = real1[t]
            group['smape']=smape[t]
            end=pd.to_datetime(end)
            group['eta']=[end - timedelta(days=(len(pre[0])-i)) for i in range(len(pre[0]))]

            t=t+1
            return group
        df=grouped.apply(lambda x: pinjie(x, pre1,end,smape))
        # 存储为 CSV 文件
