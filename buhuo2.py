...
def buhuo(df_asn,df_inbound,df_demand,inventory,fahuo,baozhuang,sku,mode=None,source=None,pre_len=None,save_model_path =None,model=None,con_len=None):
    if source=='model':
        df_demand=predict(df_demand,model,mode,con_len,pre_len,save_model_path,sku)
        out_bound=df_demand.iloc[:-pre_len,]
        df_demand=df_demand.iloc[-pre_len:,]
        print(df_demand)
        ...
        if mode=='Day':
            ...
            min_secure = quantile_95+total*0.75
            ...
            return out,min_secure,max_secure
                ...
                # 动态更新：
            #     inventory = inventory + sum(daohuo_data[:pre_len]) - sum(outbound_data[:])
            #     print(inventory)
            #     true = sum(outbound_data[:])
            #     pre = sum(demand_data2[:pre_len])
            #     if true < pre:
...
if __name__=='__main__':
    df_asn=pd.read_csv(r'E:\补货实验\b_asn.csv')
    df_inbound=pd.read_csv(r'E:\补货实验\b_inbound.csv')
    data = pd.read_csv(r'E:\补货实验\b_demand.csv')
    mode='Day'
    pre_len=14
    inventory=10000
    fahuo=20
    ...
    con_len=14
    baozhuang=20
    df_demand = make_predict_data(data, mode, sku, pre_len, con_len)
    out,_,__=buhuo(df_asn,df_inbound,df_demand,inventory,fahuo,baozhuang,sku,mode,source,pre_len,save_model_path,model,con_len)
    out.to_csv(r'E:\补货实验\结果.csv')